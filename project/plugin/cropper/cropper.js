...
var DEFAULTS = {
  ...
  // Check if the current images is a cross-origin images
  checkCrossOrigin: true,

  // Check the current images's Exif Orientation information
  ...
  // Enable to crop the images automatically when initialize
  autoCrop: true,

  // Define the percentage of automatic cropping area when initializes
  autoCropArea: 0.8,

  // Enable to move the images
  movable: true,

  // Enable to rotate the images
  rotatable: true,

  // Enable to scale the images
  scalable: true,

  // Enable to zoom the images
  zoomable: true,

  // Enable to zoom the images by dragging touch
  zoomOnTouch: true,

  // Enable to zoom the images by wheeling mouse
  zoomOnWheel: true,

  // Define zoom ratio when zoom the images by wheeling mouse
  wheelZoomRatio: 0.1,
...
function getOrientation(arrayBuffer) {
  ...
  // Only handle JPEG images (start by 0xFFD8)
...
function arrayBufferToDataURL(arrayBuffer) {
  ...
  return 'data:images/jpeg;base64,' + btoa(base64);
}
...
var render$1 = {
  ...
  renderCanvas: function renderCanvas(isChanged) {
    ...
    if (self.rotated) {
      ...
      // Computes rotated sizes with images sizes
      ...
      if (isSquareImage || aspectRatio !== canvas.aspectRatio) {
        ...
        // Computes rotated sizes with natural images sizes
...
var preview$1 = {
  initPreview: function initPreview() {
    ...
    self.$preview.each(function (i, element) {
       ...
       */
      $this.html('<img ' + crossOrigin + ' src="' + url + '" style="' + 'display:block;width:100%;height:auto;' + 'min-width:0!important;min-height:0!important;' + 'max-width:none!important;max-height:none!important;' + 'images-orientation:0deg!important;">');
    });
  },
...
var methods = {
  ...
  // Reset the images and crop box to their initial states
  ...
  /**
   * Replace the images's src and rebuild the cropper
   *
   * @param {String} url
  ...
  // Destroy the cropper and remove the instance from the images
  ...
  /**
   * Scale the abscissa of the images
   *
   * @param {Number} scaleX
   */
  ...
  /**
   * Scale the ordinate of the images
   *
   * @param {Number} scaleY
   */
  ...
  /**
   * Get the cropped area position and size data (base on the original images)
   *
  ...
  /**
   * Get the images position and size data
   *
   * @return {Object} data
   */
  ...
  /**
   * Get a canvas drawn the cropped images
   *
...
var Cropper = function () {
  ...
  createClass(Cropper, [{
    ...
    value: function build() {
      ...
      // Show the clone images if is hidden
      if (!self.isImg) {
